...
def run_test_single(Bsize, eb, state, exe, data_dir, fname_list):
    cr_list, elapsed_time_list, abs_eb_list, dx_error_list, dy_error_list = [], [], [], [], []
    for fname in fname_list:
        ...
        abs_eb_match = re.search(r"abs_eb = ([-+]?\d*\.?\d+(?:e[-+]?\d+)?)", output)
        ...
        if abs_eb_match:
            abs_eb_list.append(float(abs_eb_match.group(1)))
        if dx_error_match:
            dx_error_list.append(float(dx_error_match.group(1)))
        if dy_error_match:
            dy_error_list.append(float(dy_error_match.group(1)))
            
    return np.array(cr_list), np.array(elapsed_time_list), np.array(dx_error_list)/np.array(abs_eb_list), np.array(dy_error_list)/np.array(abs_eb_list)
    ...
    for Bsize in B_list:
        for eb in eb_list:
            ...
            for state in state_list:
                cr_list, elapsed_time_list, dx_error_list, dy_error_list = run_test_single(Bsize, eb, state, exe, DATA_DIR, fname_list)
                cr_dict[(Bsize, eb)] = cr_list[0]
                time_dict[(Bsize, eb, state)] = MAX_PLACE_HOLDER
                maxerr_dict[(Bsize, eb, state)] = (MIN_PLACE_HOLDER, MIN_PLACE_HOLDER)
                if dx_error_list.size > 0:
                    total_time = elapsed_time_list.sum()
                    dx_max_err, dy_max_err = np.max(dx_error_list), np.max(dy_error_list)
                    time_dict[(Bsize, eb, state)] = total_time
                    maxerr_dict[(Bsize, eb, state)] = (dx_max_err, dy_max_err)
...
eb_list = ["1e-1"]
B_list = ["16"]
exe_list = ["test_szx_derivative_{}d".format(d)]
...
for exe in exe_list:
    ...
    for Bsize in B_list:
        for eb in eb_list:
            ...
            print("state 0 total time = {:.6f}".format(t0))
            print("state 1 total time = {:.6f}".format(t1))
            print("state 2 total time = {:.6f}".format(t2))
