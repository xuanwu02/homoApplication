...
def run_test_single(Bsize, eb, state, exe, data_dir, fname_list):
    ...
    for fname in fname_list:
        ...
        stat_match = re.search(r"mean = ([+-]?\d*\.\d+)", output)
...
def run_test(B_list, eb_list, state_list, exe):
    ...
    for Bsize in B_list:
        for eb in eb_list:
            ...
            json_path = WORK_DIR + "setting/mean.{}d.json".format(d)
...
B_list = ["8"]
state_list = ["0", "1", "2"]
exe_list = ["test_szp_mean_{}d".format(d), "test_szx_mean_{}d".format(d), "test_szr_mean_{}d".format(d)]
DATA_DIR = "/pscratch/xli281_uksr/xwu/datasets/{}/".format(DATA_NAME)
fname_list = os.listdir(DATA_DIR)

# eb_list = ["1e-1"]
# B_list = ["4"]
# exe_list = ["test_szx_mean_{}d".format(d)]

print("Computing \033[34mMean\033[0m for {} (dims = {}, {} variables)".format(DATA_NAME, dims[:d], len(fname_list)))
...
for exe in exe_list:
    ...
    for Bsize in B_list:
        for eb in eb_list:
            ...
            print("state 0: {:.6f}".format(t0))
            print("state 1: {:.6f}".format(t1))
            print("state 2: {:.6f}".format(t2))
