...
def run_test_single(Bsize, eb, exe, data_dir, fname_list):
    ...
    for fname in fname_list:
        ...
        stat_match = re.search(r"mean of homomorphic data: ([+-]?\d*\.\d+)", output)
        cr_match = re.search(r"cr = ([+-]?\d*\.\d+)", output)
        homo_time_match = re.search(r"homomorphic mean elapsed_time:   ([+-]?\d*\.\d+)", output)
        doc_time_match = re.search(r"decompressed mean elapsed_time:   ([+-]?\d*\.\d+)", output)
...
B_list = ["4", "8", "16"]
B_list = ["8"]
exe_list = ["example_ompSZp_2D_mean"]
