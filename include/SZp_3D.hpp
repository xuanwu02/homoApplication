...
void SZp_compress_fast(
    ...
    int * col_buffer = allocateAndZero1D(block_offset_0);
    int ** prevRow_buffer = allocateAndZero2D(size.Bsize, size.dim3);
    int * prevSlice_buffer = allocateAndZero1D(size.offset_0);
                ...
                for(int i=0; i<size_x; i++){
                    ...
                    for(int j=0; j<size_y; j++){
                        ...
                        for(int k=0; k<size_z; k++){
                            ...
                            prevLeft = q;
                            if(k == size_z-1) col_buffer[index] = q;
                            (*sign_pos++) = (err_3 < 0);
    ...
    delete[] col_buffer;
    delete[] prevSlice_buffer;
    delete[] prevRow_buffer;
    free(absPredError);
    free(signFlag);
}
