...
void SZx_compress_2dMeanbased(
    ...
    for(size_t x=0; x<size.block_dim1; x++){
        ...
        for(size_t y=0; y<size.block_dim2; y++){
            ...
            int mean_quant = compute_block_mean_quant(size_x, size_y, size.dim0_offset, y_data_pos, block_buffer_pos, inver_eb);
    ...
    for(size_t x=0; x<size.block_dim1; x++){
        ...
        for(size_t y=0; y<size.block_dim2; y++){
            ...
            if(fixed_rate){
                ...
                convert2SignIntArray(signFlag, signPredError, block_size);
                for(int i=0; i<block_size; i++){
                    int64_t d = static_cast<int64_t>(signPredError[i] + block_mean);
                    uint64_t d2 = d * d;
                    quant_sum += d;
...
inline void recoverBlockRow2PrePred_FltBuffer(
    size_t x, DSize_2d size, SZxCmpBufferSet *cmpkit_set,
    unsigned char *& encode_pos, T *buffer_data_pos,
...
clock_gettime(CLOCK_REALTIME, &start2);
    ...
    for(size_t y=0; y<size.block_dim2; y++){
        ...
        if(!fixed_rate){
            for(int i=0; i<size_x; i++){
                for(int j=0; j<size_y; j++){
                    curr_buffer_pos[j] = mean_quant * errorBound;
                }
                curr_buffer_pos += buffer_dim0_offset;
            }
        }
            ...
            for(int i=0; i<size_x; i++){
                for(int j=0; j<size_y; j++){
                    curr_buffer_pos[j] = (data_pos[j] + mean_quant) * errorBound;
                }
                curr_buffer_pos += buffer_dim0_offset;
                data_pos += size_y;
            }
...
clock_gettime(CLOCK_REALTIME, &start2);
    ...
    for(size_t y=0; y<size.block_dim2; y++){
        ...
        if(!fixed_rate){
            ...
            for(int i=0; i<size_x; i++){
                for(int j=0; j<size_y; j++){
                    curr_buffer_pos[j] = data_pos[j] * errorBound;
                }
                curr_buffer_pos += buffer_dim0_offset;
                data_pos += size_y;
            }        
...
inline void dxdyProcessBlockRowPrePred_IntBuffer(
    size_t x, DSize_2d size, SZxAppBufferSet_2d<int> *buffer_set,
...
inline void dxdyProcessBlockRowPrePred_FltBuffer(
    size_t x, DSize_2d size, SZxAppBufferSet_2d<T> *buffer_set,
    SZxCmpBufferSet *cmpkit_set, T *dx_start_pos, T *dy_start_pos,
    double errorBound, bool isTopRow, bool isBottomRow
){
clock_gettime(CLOCK_REALTIME, &start2);
...
inline void dxdyProcessBlockRowPostPred_IntBuffer(
    size_t x, DSize_2d size, SZxAppBufferSet_2d<int> *buffer_set, T *Buffer_1d,
    SZxCmpBufferSet *cmpkit_set, T *dx_start_pos, T *dy_start_pos,
    double errorBound, bool isTopRow, bool isBottomRow
){
clock_gettime(CLOCK_REALTIME, &start2);
...
inline void dxdyProcessBlockRowPostPred_FltBuffer(
    size_t x, DSize_2d size, SZxAppBufferSet_2d<T> *buffer_set, T *Buffer_1d,
    SZxCmpBufferSet *cmpkit_set, T *dx_start_pos, T *dy_start_pos,
    double errorBound, bool isTopRow, bool isBottomRow
){
clock_gettime(CLOCK_REALTIME, &start2);
    ...
    if(!isTopRow) memcpy(buffer_set->currRow_data_pos-buffer_set->buffer_dim0_offset-1, prevBlockRowBottom_pos, buffer_set->buffer_dim0_offset*sizeof(int));
    if(!isBottomRow) memcpy(buffer_set->currRow_data_pos+size.Bsize*buffer_set->buffer_dim0_offset-1, nextBlockRowTop_pos, buffer_set->buffer_dim0_offset*sizeof(int));
    const T * buffer_start_pos = buffer_set->currRow_data_pos;
    for(size_t y=0; y<size.block_dim2; y++){
        int size_y = ((y+1)*size.Bsize < size.dim2) ? size.Bsize : size.dim2 - y*size.Bsize;
        T * dx_pos = dx_start_pos + y * size.Bsize;
        T * dy_pos = dy_start_pos + y * size.Bsize;
        const T * curr_row = buffer_start_pos;
        for(int i=0; i<size_x; i++){
            const T * prev_row = curr_row - buffer_set->buffer_dim0_offset;
            const T * next_row = curr_row + buffer_set->buffer_dim0_offset;
            for(int j=0; j<size_y; j++){
                dx_pos[j] = next_row[j] - prev_row[j];
                dy_pos[j] = curr_row[j+1] - curr_row[j-1];
            }
            dy_pos[0] += dy_blockmean_diff[y];
            dy_pos[size_y - 1] += dy_blockmean_diff[y+1];
            if(i == 0){
...
inline void dxdyProcessBlocksPrePred_IntBuffer(
    ...
    SZxAppBufferSet_2d<int> *buffer_set,
    ...
    for(size_t x=0; x<size.block_dim1; x++){
        ...
        if(x == 0){
            ...
            dxdyProcessBlockRowPrePred_IntBuffer<T>(x, size, buffer_set, cmpkit_set, dx_pos+offset, dy_pos+offset, errorBound, true, false);
        }else{
            ...
            if(x == size.block_dim1 - 1){
                dxdyProcessBlockRowPrePred_IntBuffer<T>(x, size, buffer_set, cmpkit_set, dx_pos+offset, dy_pos+offset, errorBound, false, true);
            }else{
                recoverBlockRow2PrePred(x+1, size, cmpkit_set, encode_pos, buffer_set->nextRow_data_pos, buffer_set->buffer_dim0_offset);
                dxdyProcessBlockRowPrePred_IntBuffer<T>(x, size, buffer_set, cmpkit_set, dx_pos+offset, dy_pos+offset, errorBound, false, false);
            }
        }
    }
...
inline void dxdyProcessBlocksPrePred_FltBuffer(
    DSize_2d& size,
    SZxCmpBufferSet *cmpkit_set, 
    SZxAppBufferSet_2d<T> *buffer_set,
    unsigned char *encode_pos,
    T *dx_pos, T *dy_pos,
    double errorBound
){
    size_t BlockRowSize = size.Bsize * size.dim2;
    T * tempBlockRow = nullptr;
    buffer_set->reset();
    extract_block_mean(cmpkit_set->compressed+FIXED_RATE_PER_BLOCK_BYTES*size.num_blocks, cmpkit_set->mean_quant_inds, size.num_blocks);
    for(size_t x=0; x<size.block_dim1; x++){
        size_t offset = x * BlockRowSize;
        if(x == 0){
            recoverBlockRow2PrePred_FltBuffer(x, size, cmpkit_set, encode_pos, buffer_set->currRow_data_pos, errorBound, buffer_set->buffer_dim0_offset);
            recoverBlockRow2PrePred_FltBuffer(x+1, size, cmpkit_set, encode_pos, buffer_set->nextRow_data_pos, errorBound, buffer_set->buffer_dim0_offset);
            dxdyProcessBlockRowPrePred_FltBuffer(x, size, buffer_set, cmpkit_set, dx_pos+offset, dy_pos+offset, errorBound, true, false);
        }else{
            rotate_buffer(buffer_set->currRow_data_pos, buffer_set->prevRow_data_pos, buffer_set->nextRow_data_pos, tempBlockRow);
            if(x == size.block_dim1 - 1){
                dxdyProcessBlockRowPrePred_FltBuffer(x, size, buffer_set, cmpkit_set, dx_pos+offset, dy_pos+offset, errorBound, false, true);
            }else{
                recoverBlockRow2PrePred_FltBuffer(x+1, size, cmpkit_set, encode_pos, buffer_set->nextRow_data_pos, errorBound, buffer_set->buffer_dim0_offset);
                dxdyProcessBlockRowPrePred_FltBuffer(x, size, buffer_set, cmpkit_set, dx_pos+offset, dy_pos+offset, errorBound, false, false);
            }
        }
    }
    printf("recover_time = %.6f\n", rec_time);
...
inline void dxdyProcessBlocksPostPred_IntBuffer(
    ...
    for(size_t x=0; x<size.block_dim1; x++){
        size_t offset = x * BlockRowSize;
        if(x == 0){
            recoverBlockRow2PostPred_IntBuffer(x, size, cmpkit_set, encode_pos, buffer_set->currRow_data_pos, buffer_set->buffer_dim0_offset);
            recoverBlockRow2PostPred_IntBuffer(x+1, size, cmpkit_set, encode_pos, buffer_set->nextRow_data_pos, buffer_set->buffer_dim0_offset);
            dxdyProcessBlockRowPostPred_IntBuffer(x, size, buffer_set, Buffer_1d, cmpkit_set, dx_pos+offset, dy_pos+offset, errorBound, true, false);
        }else{
            rotate_buffer(buffer_set->currRow_data_pos, buffer_set->prevRow_data_pos, buffer_set->nextRow_data_pos, tempBlockRow);
            if(x == size.block_dim1 - 1){
                dxdyProcessBlockRowPostPred_IntBuffer(x, size, buffer_set, Buffer_1d, cmpkit_set, dx_pos+offset, dy_pos+offset, errorBound, false, true);
            }else{
                recoverBlockRow2PostPred_IntBuffer(x+1, size, cmpkit_set, encode_pos, buffer_set->nextRow_data_pos, buffer_set->buffer_dim0_offset);
                dxdyProcessBlockRowPostPred_IntBuffer(x, size, buffer_set, Buffer_1d, cmpkit_set, dx_pos+offset, dy_pos+offset, errorBound, false, false);
            }
        }
    }
    printf("recover_time = %.6f\n", rec_time);
...
inline void dxdyProcessBlocksPostPred_FltBuffer(
    ...
    for(size_t x=0; x<size.block_dim1; x++){
        size_t offset = x * BlockRowSize;
        if(x == 0){
            recoverBlockRow2PostPred_FltBuffer(x, size, cmpkit_set, encode_pos, buffer_set->currRow_data_pos, errorBound, buffer_set->buffer_dim0_offset);
            recoverBlockRow2PostPred_FltBuffer(x+1, size, cmpkit_set, encode_pos, buffer_set->nextRow_data_pos, errorBound, buffer_set->buffer_dim0_offset);
            dxdyProcessBlockRowPostPred_FltBuffer(x, size, buffer_set, Buffer_1d, cmpkit_set, dx_pos+offset, dy_pos+offset, errorBound, true, false);
        }else{
            rotate_buffer(buffer_set->currRow_data_pos, buffer_set->prevRow_data_pos, buffer_set->nextRow_data_pos, tempBlockRow);
            if(x == size.block_dim1 - 1){
                dxdyProcessBlockRowPostPred_FltBuffer(x, size, buffer_set, Buffer_1d, cmpkit_set, dx_pos+offset, dy_pos+offset, errorBound, false, true);
            }else{
                recoverBlockRow2PostPred_FltBuffer(x+1, size, cmpkit_set, encode_pos, buffer_set->nextRow_data_pos, errorBound, buffer_set->buffer_dim0_offset);
                dxdyProcessBlockRowPostPred_FltBuffer(x, size, buffer_set, Buffer_1d, cmpkit_set, dx_pos+offset, dy_pos+offset, errorBound, false, false);
            }
        }
    }
    printf("recover_time = %.6f\n", rec_time);
...
template <class T>
void SZx_dxdy_2dMeanbased_IntBuffer(
    unsigned char *cmpData, size_t dim1, size_t dim2,
    int blockSideLength, double errorBound,
    ...
    SZxAppBufferSet_2d<int> * buffer_set = new SZxAppBufferSet_2d<int>(buffer_dim1, buffer_dim2, Buffer_2d, appType::CENTRALDIFF);
    ...
    switch(state){
        case decmpState::postPred:{
            dxdyProcessBlocksPostPred_IntBuffer(size, cmpkit_set, Buffer_1d, buffer_set, encode_pos, dx_pos, dy_pos, errorBound);
            break;
        }
        case decmpState::prePred:{
...
template <class T>
void SZx_dxdy_2dMeanbased_FltBuffer(
    unsigned char *cmpData, size_t dim1, size_t dim2,
    int blockSideLength, double errorBound,
    T *dx_result, T *dy_result, decmpState state
){
    ...
    int * blocks_mean_quant = (int *)malloc(size.num_blocks * sizeof(int));
    T * Buffer_1d = (T *)malloc(buffer_dim2 * 3 * sizeof(T));
    T * Buffer_2d = (T *)malloc(buffer_dim1 * buffer_dim2 * 3 * sizeof(T));
    T * decData = (T *)malloc(size.nbEle * sizeof(T));
    SZxAppBufferSet_2d<T> * buffer_set = new SZxAppBufferSet_2d<T>(buffer_dim1, buffer_dim2, Buffer_2d, appType::CENTRALDIFF);
    SZxCmpBufferSet * cmpkit_set = new SZxCmpBufferSet(cmpData, blocks_mean_quant, signPredError, signFlag);
    unsigned char * encode_pos = cmpData + (FIXED_RATE_PER_BLOCK_BYTES + INT_BYTES) * size.num_blocks;
    T * dx_pos = dx_result;
    T * dy_pos = dy_result;
    ...
    switch(state){
        case decmpState::postPred:{
            dxdyProcessBlocksPostPred_FltBuffer(size, cmpkit_set, Buffer_1d, buffer_set, encode_pos, dx_pos, dy_pos, errorBound);
            break;
        }
        case decmpState::prePred:{
...
inline void laplacianProcessBlockRowPrePred_IntBuffer(
    size_t x, DSize_2d size, SZxAppBufferSet_2d<int> *buffer_set,
    SZxCmpBufferSet *cmpkit_set, T *result_start_pos,
    double errorBound, bool isTopRow, bool isBottomRow
){
clock_gettime(CLOCK_REALTIME, &start2);
    int size_x = ((x+1) * size.Bsize < size.dim1) ? size.Bsize : size.dim1 - x * size.Bsize;
...
clock_gettime(CLOCK_REALTIME, &start2);
    ...
    for(size_t y=0; y<size.block_dim2; y++){
        ...
        for(int i=0; i<size_x; i++){
            ...
            laplacian_pos += size.dim0_offset;
            curr_row += buffer_set->buffer_dim0_offset;
        }
        buffer_start_pos += size.Bsize;
    }
...
inline void laplacianProcessBlocksPrePred_IntBuffer(
    ...
    for(size_t x=0; x<size.block_dim1; x++){
        ...
        if(x == 0){
            recoverBlockRow2PrePred(x, size, cmpkit_set, encode_pos, buffer_set->currRow_data_pos, buffer_set->buffer_dim0_offset);
            recoverBlockRow2PrePred(x+1, size, cmpkit_set, encode_pos, buffer_set->nextRow_data_pos, buffer_set->buffer_dim0_offset);
            laplacianProcessBlockRowPrePred_IntBuffer<T>(x, size, buffer_set, cmpkit_set, result_pos+offset, errorBound, true, false);
        }else{
            rotate_buffer(buffer_set->currRow_data_pos, buffer_set->prevRow_data_pos, buffer_set->nextRow_data_pos, tempBlockRow);
            if(x == size.block_dim1 - 1){
                laplacianProcessBlockRowPrePred_IntBuffer<T>(x, size, buffer_set, cmpkit_set, result_pos+offset, errorBound, false, true);
            }else{
                recoverBlockRow2PrePred(x+1, size, cmpkit_set, encode_pos, buffer_set->nextRow_data_pos, buffer_set->buffer_dim0_offset);
                laplacianProcessBlockRowPrePred_IntBuffer<T>(x, size, buffer_set, cmpkit_set, result_pos+offset, errorBound, false, false);
            }
        }
    }
    printf("recover_time = %.6f\n", rec_time);
...
inline void laplacianProcessBlocksPostPred_IntBuffer(
    ...
    for(size_t x=0; x<size.block_dim1; x++){
        ...
        if(x == 0){
            recoverBlockRow2PostPred_IntBuffer(x, size, cmpkit_set, encode_pos, buffer_set->currRow_data_pos, buffer_set->buffer_dim0_offset);
            recoverBlockRow2PostPred_IntBuffer(x+1, size, cmpkit_set, encode_pos, buffer_set->nextRow_data_pos, buffer_set->buffer_dim0_offset);
            laplacianProcessBlockRowPostPred_IntBuffer(x, size, buffer_set, Buffer_1d, cmpkit_set, result_pos+offset, errorBound, true, false);
        }else{
            rotate_buffer(buffer_set->currRow_data_pos, buffer_set->prevRow_data_pos, buffer_set->nextRow_data_pos, tempBlockRow);
            if(x == size.block_dim1 - 1){
                laplacianProcessBlockRowPostPred_IntBuffer(x, size, buffer_set, Buffer_1d, cmpkit_set, result_pos+offset, errorBound, false, true);
            }else{
                recoverBlockRow2PostPred_IntBuffer(x+1, size, cmpkit_set, encode_pos, buffer_set->nextRow_data_pos, buffer_set->buffer_dim0_offset);
                laplacianProcessBlockRowPostPred_IntBuffer(x, size, buffer_set, Buffer_1d, cmpkit_set, result_pos+offset, errorBound, false, false);
            }
        }
    }
    printf("recover_time = %.6f\n", rec_time);
...
template <class T>
void SZx_laplacian_2dMeanbased_IntBuffer(
    unsigned char *cmpData, size_t dim1, size_t dim2,
    int blockSideLength, double errorBound,
    ...
    switch(state){
        case decmpState::postPred:{
            laplacianProcessBlocksPostPred_IntBuffer(size, cmpkit_set, Buffer_1d, buffer_set, encode_pos, laplacian_pos, errorBound);
            break;
        }
        case decmpState::prePred:{
...
inline void heatdisProcessCompressBlockRowPrePred(
    ...
    SZxCmpBufferSet *cmpkit_set, SZxAppBufferSet_2d<int> *buffer_set,
...
inline void heatdisUpdatePrePred(
    ...
    SZxAppBufferSet_2d<int> *buffer_set, TempInfo2D& temp_info,
...
void SZx_heatdis_2dMeanbased(
    ...
    SZxAppBufferSet_2d<int> * buffer_set = new SZxAppBufferSet_2d<int>(buffer_dim1, buffer_dim2, Buffer_2d, appType::HEATDIS);
