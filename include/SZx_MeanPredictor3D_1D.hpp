#ifndef _SZX_MEAN_PREDICTOR_3D_1D_HPP
#define _SZX_MEAN_PREDICTOR_3D_1D_HPP
...
template <class T>
void SZx_compress3D_1dMeanbased(
    ...
    for(size_t x=0; x<size.block_dim1; x++){
        ...
        for(size_t y=0; y<size.block_dim2; y++){
            ...
            for(size_t z=0; z<size.block_dim3; z++){
                ...
                int mean_quant = compute_block_mean_quant(block_size, z_data_pos, block_quant_inds, inver_eb);
...
template <class T>
void SZx_decompress3D_1dMeanbased(
...
// double SZx_mean3D_1d_postPred2(
...
double SZx_mean3D_1d_postPred(
    ...
    unsigned char * signFlag = (unsigned char *)malloc(size.max_num_block_elements*sizeof(unsigned char));
    int * blocks_mean_quant = (int *)malloc(size.num_blocks * sizeof(int));
    int64_t global_mean = compute_integer_mean_3d_1d<int64_t>(size, qmean_pos, blocks_mean_quant);
    int block_ind = 0;
    int64_t quant_sum = 0;
    for(size_t x=0; x<size.block_dim1; x++){
        for(size_t y=0; y<size.block_dim2; y++){
            for(size_t z=0; z<size.block_dim3; z++){
                int block_size = ((z+1)*size.Bsize < size.dim2) ? size.Bsize : size.dim2 - z*size.Bsize;
...
template <class T>
double SZx_mean3D_1d_decOp(
    unsigned char *cmpData, size_t dim1, size_t dim2, size_t dim3,
    T *decData, int blockSideLength, double errorBound
){
    size_t nbEle = dim1 * dim2 * dim3;
    SZx_decompress3D_1dMeanbased(decData, cmpData, dim1, dim2, dim3, blockSideLength, errorBound);
    double mean = 0;
...
double SZx_mean3D_1d(
    ...
    switch(state){
        case decmpState::full:{
            mean = SZx_mean3D_1d_decOp(cmpData, dim1, dim2, dim3, decData, blockSideLength, errorBound);            
            break;
        }
        case decmpState::prePred:{
            // mean = SZx_mean3D_1d_prePred(cmpData, dim1, dim2, dim3, blockSideLength, errorBound);            
            break;
        }
        case decmpState::postPred:{
            mean = SZx_mean3D_1d_postPred(cmpData, dim1, dim2, dim3, blockSideLength, errorBound);            
            break;
        }
    }
...
double SZx_variance3D_1d_postPred(
    unsigned char *cmpData, size_t dim1, size_t dim2, size_t dim3,
    int blockSideLength, double errorBound
){
    DSize_3d1d size(dim1, dim2, dim3, blockSideLength);
...
template <class T>
double SZx_variance3D_1d_decOp(
    unsigned char *cmpData, size_t dim1, size_t dim2, size_t dim3,
    T *decData, int blockSideLength, double errorBound
){
    size_t nbEle = dim1 * dim2 * dim3;
...
double SZx_variance3D_1d(
    ...
    switch(state){
        case decmpState::full:{
            var = SZx_variance3D_1d_decOp(cmpData, dim1, dim2, dim3, decData, blockSideLength, errorBound);            
            break;
        }
        case decmpState::prePred:{
            break;
        ...
        case decmpState::postPred:{
            var = SZx_variance3D_1d_postPred(cmpData, dim1, dim2, dim3, blockSideLength, errorBound);            
            break;
        }
    }
    clock_gettime(CLOCK_REALTIME, &end);
...
inline void recoverBlockPlane2PrePred(
    size_t x, DSize_3d1d size, SZxCmpBufferSet *cmpkit_set,
...
clock_gettime(CLOCK_REALTIME, &start2);
    ...
    for(int i=0; i<size_x; i++){
        for(size_t y=0; y<size.block_dim2; y++){
            ...
            buffer_start_pos += buffer_dim1_offset - size.Bsize * size.block_dim3;
        }
        buffer_start_pos += buffer_dim0_offset - size.dim2 * buffer_dim1_offset;
    }
...
inline void dxdydzProcessBlockPlanePrePred(
    size_t x, DSize_3d1d& size, SZxAppBufferSet_3d1d *buffer_set,
...
inline void dxdydzProcessBlocksPrePred(
    ...
    SZxAppBufferSet_3d1d *buffer_set,
...
void SZx_dxdydz_1dMeanbased(
    ...
    unsigned char * signFlag = (unsigned char *)malloc(size.max_num_block_elements*sizeof(unsigned char));
    SZxAppBufferSet_3d1d * buffer_set = new SZxAppBufferSet_3d1d(buffer_dim1, buffer_dim2, buffer_dim3, Buffer_3d, appType::CENTRALDIFF);
...
template <class T>
void SZx_laplacian3D_1dMeanbased(
    unsigned char *cmpData, size_t dim1, size_t dim2, size_t dim3,
    int blockSideLength, double errorBound,
