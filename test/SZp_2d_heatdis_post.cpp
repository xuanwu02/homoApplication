...
int main(int argc, char **argv)
{
    ...
    SZp_heatdis_2dLorenzo(cmpData, dim1, dim2, blockSideLength, max_iter, cmpSize, src_temp, wall_temp, ratio, errorBound, decmpState::postPred);
    clock_gettime(CLOCK_REALTIME, &end);
    elapsed_time = get_elapsed_time(start, end);
    printf("  recover_time = %.6f\n", postPred_decmp_time);
    printf("  update_time = %.6f\n", postPred_op_time);
    printf("  compress_time = %.6f\n", postPred_cmp_time);
    ...
    SZp_decompress_2dLorenzo(decData, compressed.data(), dim1, dim2, blockSideLength, errorBound);
