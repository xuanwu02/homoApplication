...
int main(int argc, char **argv)
{
    ...
    T * curl_result = (T *)malloc(nbEle * sizeof(T));
    T * ref_curl_result = (T *)malloc(nbEle * sizeof(T));

    size_t cmpSize = 0, compressed_size = 0;
    for(i=0; i<dim; i++){
        SZx_compress(oriData[i], cmpData[i], dim1, dim2, blockSideLength, eb, compressed_size);
    ...
    SZx_curl(cmpData, dim1, dim2, blockSideLength, eb, curl_result, intToDecmpState(stateType));
    for(i=0; i<dim; i++){
        SZx_decompress(decData[i], cmpData[i], dim1, dim2, blockSideLength, eb);
    }
    compute_curl_2d(dim1, dim2, decData[0], decData[1], ref_curl_result);
    double err = verify_dxdy(ref_curl_result, curl_result, dim1, dim2);
