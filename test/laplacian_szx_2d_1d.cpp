...
#include <ctime>
#include <cassert>
#include "SZx_MeanPredictor2D_1D.hpp"
#include "utils.hpp"
...
int main(int argc, char **argv)
{
    ...
    size_t cmpSize = 0;
    SZx_compress2D_1dMeanbased(oriData, cmpData, dim1, dim2, blockSideLength, eb, cmpSize);
    printf("cr = %.2f\n", 1.0 * nbEle * sizeof(T) / cmpSize);

    SZx_laplacian2D_1dMeanbased(cmpData, dim1, dim2, blockSideLength, eb, laplacian_result, intToDecmpState(stateType));

    SZx_decompress2D_1dMeanbased(decData, cmpData, dim1, dim2, blockSideLength, eb);
    compute_laplacian_2d(dim1, dim2, decData, ref_laplacian_result);
    double err;
    err = verify_dxdy(ref_laplacian_result, laplacian_result, dim1, dim2);
