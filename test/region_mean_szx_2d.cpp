...
int main(int argc, char **argv)
{
    ...
    double mean = SZx_region_mean(cmpData, dim1, dim2, decData, ratio, blockSideLength, eb, intToDecmpState(stateType));
    printf("mean = %.6f\n", mean);

    SZx_decompress(decData, cmpData, dim1, dim2, blockSideLength, eb);
    double doc_mean = compute_region_mean(dim1, dim2, blockSideLength, ratio, decData);
