...
int main(int argc, char **argv)
{
    ...
    double ratio = atof(argv[argv_id++]);
    ...
    double mean = SZx_region_mean(cmpData, dim1, dim2, dim3, decData, ratio, blockSideLength, eb, intToDecmpState(stateType));
    printf("mean = %.6f\n", mean);

    SZx_decompress(decData, cmpData, dim1, dim2, dim3, blockSideLength, eb);
    double doc_mean = compute_region_mean(dim1, dim2, dim3, blockSideLength, ratio, decData);
