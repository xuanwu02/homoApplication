...
int main(int argc, char **argv)
{
    ...
    size_t cmpSize = 0;
    SZp_compress_1dLorenzo(oriData, cmpData, s.dim1, s.dim2, s.B, s.eb, cmpSize);
    printf("cr = %.2f\n", 1.0 * nbEle * sizeof(T) / cmpSize);

    SZp_dxdy_1dLorenzo(cmpData, s.dim1, s.dim2, s.B, s.eb, dx_result, dy_result, intToDecmpState(s.stateType));

    SZp_decompress_1dLorenzo(decData, cmpData, s.dim1, s.dim2, s.B, s.eb);
