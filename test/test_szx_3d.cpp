...
int main(int argc, char **argv)
{
    ...
    switch(opt){
        ...
        case 1:{
            clock_gettime(CLOCK_REALTIME, &start);
            SZx_decompress_to_PrePred_3dMeanbased(decQuant, cmpData, s.dim1, s.dim2, s.dim3, s.B, s.eb);
            clock_gettime(CLOCK_REALTIME, &end);
            elapsed_time = get_elapsed_time(start, end);
        ...
        case 2:{
            clock_gettime(CLOCK_REALTIME, &start);
            SZx_decompress_to_PostPred_3dMeanbased(decQuant, cmpData, s.dim1, s.dim2, s.dim3, s.B, s.eb);
            clock_gettime(CLOCK_REALTIME, &end);
            elapsed_time = get_elapsed_time(start, end);
